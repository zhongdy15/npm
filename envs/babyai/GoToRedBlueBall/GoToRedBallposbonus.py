...
class GoToRedBallPositionBonus(GoToRedBlueBall):
    ...
    def gen_mission(self):
        ...
        for dist in dists:
            if dist.type == "ball" and (dist.color == "blue" or dist.color == "red"):
                raise RejectSampling("can only have one blue and one red ball")


        # color = self._rand_elem(["red", "blue"])
        # obj, _ = self.add_object(0, 0, "ball", color)
        obj1, _ = self.add_object(0, 0, "ball", "red")
        obj2, _ = self.add_object(0, 0, "ball", "blue")
        # Make sure no unblocking is required
        self.check_objs_reachable()

        # choose obj1 as goal
        self.instrs = GoToInstr(ObjDesc(obj1.type, obj1.color))
