...
def test(config,log_path):
    ...
    for model_name in zip_file_list:
        ...
        mean, std = evaluate_policy_and_save(model, env, save_path=os.path.join(log_path, model_name.rstrip(".zip") + ".npy"), deterministic=False)
        print("model:" + model_name)
        print("mean:" + str(mean) + " std:" + str(std))


if __name__ == '__main__':
    experiment_name = "unlockpickupuncertaingoalsactionbonus-v0_PPO_n-1_2023-03-19-19-49-37"
